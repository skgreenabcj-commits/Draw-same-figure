...
/* ====================================================================
   § 11. API 送信コア（responseMimeType 自動切替ロジック）
==================================================================== */
 ...
 *   noMime キャッシュなし → responseMimeType 付きで送信
 *     → HTTP 400: _setNoMimeCache でフラグ保存 → テキストモードで即再送
 *     → その他エラー: status を返す
 *   noMime キャッシュあり → テキストモードで送信
 *
 * 「即再送」は attempt カウントを消費しない。
 *
 * @param {string}  model
 * @param {string}  apiKey
 * @param {string}  prompt
...
async function _callApi(model, apiKey, prompt, forcePlain = false) {
  ...
  if (status === 400 && !forcePlain) {
    console.warn(`[gemini] HTTP 400 受信 → ${model} は responseMimeType 非対応と判定`);
    _setNoMimeCache(model);
    // forcePlain=true で即再送（attempt カウントを消費しない）
  ...
  let raw;
  if (useJsonMode) {
    // JSON モード: レスポンス全体をそのままパース試行
    try {
      raw = JSON.parse(text);
      if (!Array.isArray(raw)) raw = null;
    } catch (_) {
      // JSON モードでも稀にマークダウンが混入する場合はテキスト抽出にフォールバック
      raw = _extractJsonArray(text);
    }
  } else {
    // テキストモード: スタックベースのパーサーで抽出
    raw = _extractJsonArray(text);
  }
 ...
 *
 * 生成フロー:
 *   1. resolveModel() で使用モデルを決定
 *   2. MAX_ATTEMPTS 回まで API 呼び出しを試みる
 *      - 429（レート制限）: attempt-- して待機後リトライ（枠を消費しない）
 *      - 403/401: 認証エラー → 即座に throw
 *      - 400（forcePlain 後）: モデル問題の可能性 → break
 *      - 取得した候補を _normalise + _validate でフィルタ
 *   3. 5問揃ったら返す
...
async function generateProblems(level, count, apiKey) {
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS && validated.length < count; attempt++) {
    ...
    // ── エラーハンドリング ─────────────────────────────────────────
    if (status === 429) {
      // レート制限: attempt を戻してウェイト後リトライ（枠を消費しない）
      attempt--;
      const wait = 2000 + Math.random() * 1000;
      console.warn(`[gemini] 429 レート制限 → ${Math.round(wait)}ms 待機後リトライ`);
